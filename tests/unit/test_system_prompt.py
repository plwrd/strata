"""Distill Qwen 7B defaults and SystemPrompt.md wiring."""
...
def test_default_settings_point_at_distill_qwen_7b() -> None:
    assert AppSettings().default_provider == "ollama"
    assert AppSettings().default_model == DEFAULT_LOCAL_MODEL
    assert is_distill_qwen_7b(AppSettings().default_model)
...
def test_blank_and_default_resolve_to_distill_qwen_7b() -> None:
    assert resolve_model("") == DEFAULT_LOCAL_MODEL
    assert resolve_model("default") == DEFAULT_LOCAL_MODEL
    assert resolve_model("  ", default_model="deepseek-r1:7b") == DEFAULT_LOCAL_MODEL
