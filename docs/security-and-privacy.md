...
| WebView2 research pane (8) | The same pane on Edge's engine (Windows). A second browser runtime in a process Strata launches, with its own cookie store | The same boundary as the Qt pane and for the same reasons: **no host object and no web-message channel** (`AreHostObjectsAllowed` and `IsWebMessageEnabled` both off), navigation restricted to `http`/`https`, default context menus and devtools off, `window.open`/`target=_blank` handled in-pane so no top-level browser window is created that Strata could not exclude from capture. Its browser process is swept by PID for capture exclusion alongside our own windows. Residual: a second at-rest cookie store, same footing as the Qt pane's (T-34) |
| Browser extensions in the WebView2 pane (8) | Third-party code, chosen by the user, that reads every page the pane visits and may send what it reads anywhere | **Empty by default**; the extension subsystem is not even enabled in the browser process unless a folder is configured. Each one is added by an explicit native folder picker, capped at ten, and the loaded names — and any folder that failed to load — are reported in the Research panel rather than assumed. An extension inherits the pane's boundary, so it has no channel to Python and no path to the workspace or a locked layer. Residual, and stated plainly to the user: an extension can exfiltrate what is *in the pane*, which is the page the user chose to research. This is a discretion decision the user makes per extension, not a boundary Strata enforces |
