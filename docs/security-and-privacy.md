...
| `browser.set_blur` + injected stylesheet | 8 | Injects a `<style>` into the embedded pane (only) that blurs `img, video, canvas`. A discretion feature, not a security boundary — a page can override the filter, and blur does nothing against a capture pipeline (that is *Hidden for sharing*, T-30-adjacent). The one integrity concern is the injection itself: the blur radius is `int()`-coerced and clamped 1–100 and the stylesheet is embedded as a JSON string literal, so a value can carry neither CSS nor script into the page — verified by `tests/unit/test_browser_blur.py`. Runs in the pane's main world; adds no capability a page did not already have. The Chrome backend is never restyled (`blur_supported=false`) |
