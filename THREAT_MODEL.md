...
| T-34 | **Research browser as a read channel** — a browser Strata can read is a browser Strata can read *everything* in, including logged-in pages, and Strata can be asked to send that text to a model. The **embedded pane** additionally stores the user's sign-in cookies inside the app's data directory; the **Chrome backend** additionally opens a loopback DevTools port | ADV-5, ADV-7, ADV-1 | I, E | Partially mitigated | **Off by default** (`settings.browser_control_enabled`), for both backends. Reading is per-page and user-initiated — no background crawl, no "read every tab". Scraped text is stored as an untrusted capture (T-15, T-16 apply in full) and reaches a provider only through the unchanged policy gate. Search is navigation, not a Strata request, so no engine sees an outbound call from the app. **Embedded pane:** its own `QWebEngineProfile`, sharing no cookie jar, cache or storage path with the app profile (which stays `NoPersistentCookies`); no `QWebChannel` on that page, so nothing it loads has a bridge; navigation restricted to `http`/`https`, so it can reach neither `strata://` nor `file://`. **Chrome backend:** port bound to `127.0.0.1`, non-loopback debugging addresses refused by the client, Strata's own profile directory unless the user redirects `browser_profile_path`. Both are closed when the window closes. | **The pane's cookie store is plaintext-adjacent at rest**: a signed-in research session lives in the app data dir under whatever the OS gives it, not under a layer key — an attacker with the disk (ADV-1) gets those sessions, exactly as they would from any browser profile. And **a loopback debugging port is not an access-control boundary**: while the Chrome backend's window is open, any process running as the same user can drive it. That is a property of Chrome's protocol, not something Strata can fix, and it is why that backend is not the default. A user who points Strata at their main profile has made a larger trade, and the setting says so. | M11 |
...
| T-30 | **Renderer compromise escalating to the host** — an XSS or a Chromium bug in the WebEngine renderer tries to reach the filesystem, shell, or network | ADV-5, ADV-6 | E | Mitigated | The bridge exposes **no** arbitrary Python, shell, filesystem, or network primitive — only enumerated, Pydantic-validated, 1 MiB-capped JSON operations on fourteen feature-scoped objects (A-007). No crypto and no keys in the renderer. Strict CSP; external navigation blocked; DevTools and the remote debugging port compiled out of release builds. `strata://` is used instead of a localhost HTTP server precisely so that **Strata itself listens on no local port** (A-006). *Qualifier:* the embedded browser pane opens no port at all, but when research is switched to the **Chrome backend** (off by default, not the default backend), the browser Strata launches listens on `127.0.0.1:<debug port>` for as long as that window is open — a port in a different process, reachable by anything running as the user. Strata closes that browser when the window closes. See T-34. | A renderer sandbox escape (a Chromium 0-day) lands the attacker in the Python process, which holds the keys — at which point this is T-04/ADV-7 and out of scope. Keeping Qt WebEngine patched is a release requirement. | M1 |
...
| T-35 | **Encrypted web archive** (Ctrl+Alt+F) — saved pages and videos could leak through a plaintext download, a temp file, the engine's cache, or a vault page being reachable from the web | ADV-1, ADV-5 | I, T | Mitigated | Saved items are **stream objects** in a private layer, encrypted chunk by chunk as they arrive — no plaintext download, temp file or cache (encryption format §3.7). Saving refuses unless a private layer is unlocked; there is no plain-storage fallback. Items are served back from a reserved, never-resolving host (`saved.strata.invalid`) by answering WebView2's resource requests in-process: decrypted on read, `Cache-Control: no-store`, and only to a document the pane itself opened — a web page's request for the vault gets 403. Saved pages are shown under a CSP with no script and no network. Locking the layer closes every open reader, so a playing video stops, and aborts a save in progress (its partial file is ciphertext and is removed). Cookies sent to fetch a video are matched per URL, never pooled across hosts. **Streamed video (YouTube, X):** yt-dlp finds the streams using the pane's cookies loaded into its *in-memory* jar (no cookie file, `cachedir` off); ffmpeg joins the tracks into a fragmented MP4 on **stdout** only — it is given no output file — and each chunk is encrypted as it is read from the pipe. Cookies are kept off ffmpeg's command line (the signed stream URLs need none). DRM and live streams are refused. **SSRF:** every video URL — and every redirect hop — is refused if it resolves to a loopback, private, link-local or reserved address (shared `net_guard`, as URL import; opt-in `web_archive_allow_private_addresses` for a NAS), and ffmpeg inputs are limited to `-protocol_whitelist https,http,tls,tcp,crypto,hls`. | While unlocked, decrypted bytes are in process memory (as for every private object, T-04). The *browsing* profile of the pane (its HTTP cache, cookies) is Edge's and is not under a layer key (T-34). yt-dlp and ffmpeg are third-party code run with the user's session for the saved site — each in its own process under a Windows Job Object (memory ceiling; ffmpeg may start no other program; killed with Strata), yt-dlp's reply re-validated by Strata — but still as the user, with the user's file access (not an AppContainer); sites change and yt-dlp must be kept current; a site may refuse (YouTube's bot check needs a signed-in pane). Plaintext of a streamed video passes through ffmpeg's process memory. DRM media is out of scope. HLS segment URLs inside a playlist are fetched by ffmpeg itself and are not address-checked per segment; DNS rebinding remains (as T-34). File sizes leak to chunk granularity. | — |
...
- The research browser gains a capability beyond navigate/list/read-one-page (T-34).
- The embedded pane's profile, navigation policy, or bridge exposure changes (T-30, T-34).
